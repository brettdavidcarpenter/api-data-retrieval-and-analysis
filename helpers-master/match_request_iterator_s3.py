...
def iterate(name):
    ...
    with open (filename,encoding='utf-8') as csvinput:
        with open("business_names_output2.csv",'w',encoding='utf-8') as csvoutput:
                    ...
                    try:
                        if results_json["risks"] is not None:
                            ...
                            while i < len(results_json["risks"]):
                                row.append(results_json["risks"][i]['group_code'])
                                row.append(results_json["risks"][i]['ruleset_code'])
                                row.append(results_json["risks"][i]['risk_name'])
                                row.append(results_json["risks"][i]['regex'])
                                row.append(results_json["risks"][i]['extracted'])
                                i +=1
                            #list_risks()
                    except Exception as f:
                        print()
